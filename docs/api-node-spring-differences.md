...
| Request code | `POST /api/auth/request-code` | `POST /api/v1/auth/request-code` | Both require an email and purpose; production requires real SMTP and never echoes a code. |
...
| Classroom | `GET/POST /api/assignments`, `/api/teacher/*`, `/api/learning-snapshot` | `GET /api/v1/classroom/scripts`, `POST /api/v1/classroom/sessions`, owned session/actions, and `/api/v1/learning/*` | Node stores aggregate snapshots and teacher assignments in SQLite. Spring stores reviewed scripts, state transitions, evidence, and per-chapter progress in MySQL. |
