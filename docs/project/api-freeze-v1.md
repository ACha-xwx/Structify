...
`POST /api/v1/admin/model-config/test` has no caller-supplied body. It uses the saved configuration to send a pinned HTTPS `POST` to `{baseUrl}/chat/completions` with the saved `model`, one `Connection test` user message, `max_tokens: 16`, `temperature: 0`, and `stream: false`. A 2xx result is successful only when the body is an OpenAI-compatible chat-completion envelope: a non-empty `choices` array whose first entry carries a `message` object. Visible answer text is deliberately not required, because a reasoning model may answer with `reasoning_content` or an empty `content` once the capped probe stops generation, and that must not be misreported as a broken configuration. A 2xx response with a missing, empty, malformed, or oversized completion body returns `{connected:false, code:"CONNECTION_RESPONSE_INVALID"}`. Other controlled outcomes are `CONNECTION_FAILED`, `REDIRECT_REJECTED`, and the upstream rejections `UPSTREAM_AUTHENTICATION_REJECTED` (401/403, wrong credential), `UPSTREAM_PAYMENT_REQUIRED` (402, empty provider balance), `UPSTREAM_RATE_LIMITED` (429), and `UPSTREAM_REJECTED` for every other non-2xx status.
